 ...
 */
public class GXBaseServiceImpl<T, M extends GXBaseMapper<T, E>, D extends GXBaseDao<M, T>, E> implements GXBaseService<T, M, D, E> {
    @Autowired
    protected D baseDao;
    ...
    public <R> R getSingleFieldValueByEntity(T entity, String path, Class<R> type, R defaultValue, int coreModelId) {
        ...
        if (index == -1) {
            if (null != dbField.get(path)) {
                GXCommonUtils.getLogger(GXBaseServiceImpl.class).error("当前用户无权访问{}字段的数据...", path);
                return null;
            }
        ...
        if (!dict.isEmpty() && null != dict.get(subField)) {
            GXCommonUtils.getLogger(GXBaseServiceImpl.class).error("当前用户无权访问{}.{}字段的数据...", mainField, subField);
            return null;
        }
